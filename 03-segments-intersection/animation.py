...
def find_intersections(S):
    ...
    def find_events(s_l: Segment, s_r: Segment, p: SegmPoint):
        ...
        if intersection:
            intersect_above = intersection.y < p.y
            ...
            #print(is_not_event)
            if intersection and (intersect_above or intersect_left) and is_not_event:
            #if intersection and (intersect_above or intersect_left):
    ...
    while Q.root:
        ...
        print(f"processing event {point}")

        for segment in segments:
            x, y = separate([segment.superior, segment.inferior])
            ax1.plot(x, y, color='grey')

        if first_time:
            first_time = False
            xlim = ax1.get_xlim()
        ax1.plot(list(xlim), [point.y, point.y], color="steelblue")
        plt.scatter([point.x], [point.y], color="mediumpurple")

        for intersection in R:
            plt.scatter([intersection.point.x], [intersection.point.y], color="palevioletred")
...
if __name__ == "__main__":
    ...
    #data = read_file('test-cases/algorithmical/0.in')
